...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_two_loops(self, X):
        ...
        - dists: A numpy array of shape (num_test, num_train) where dists[i, j]
          ...
          point.
        """
        print("Check Dim",self.X_train.shape,X.shape)
        num_test = X.shape[0]
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dist = np.sqrt(np.sum(np.power(X[i]-self.X_train[j],2)))
                # print(dist)
                dists[i,j] = dist
                #                 pass
    ...
    def compute_L2_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            test1 = X[i] - self.X_train
            test2 = np.power(test1,2)
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        a_s = np.square(X)        
        b_s = np.square(self.X_train)
        two_ab = 2 * np.dot(X, self.X_train.T)

        a_s_sum = a_s.sum(axis=1)
        b_s_sum = b_s.sum(axis=1)

        temp =  a_s_sum[:,np.newaxis] + b_s_sum # broadcasting
        
        dists = np.sqrt(temp - two_ab)
    ...
    def compute_L1_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dist = np.sum(X[i]-self.X_train[j])
                # print(dist)
                dists[i,j] = dist
    ...
    def compute_L1_distances_no_loops(self, X):
        ...
        a_sum = a.sum(axis=1)
        b_sum = b.sum(axis=1)
        dists =  a_sum[:,np.newaxis] - b_sum # broadcasting

...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        ## hidden Layer1 Forward Pass
        fcl1 = np.dot(X, W1) + b1
        hidden_layer_1 = np.maximum(0,fcl1)

        ## hidden Layer2 Forward Pass  
        fcl2 = np.dot(hidden_layer_1,W2) + b2
        hidden_layer_2 = np.maximum(0,fcl2)

        ## Softmax Layer
        fcl3 = np.dot(hidden_layer_2, W3) + b3
        # hidden_layer_3 = np.maximum(0, fcl3)
        ...
        regularization_loss = np.sum(np.power(W1,2))  + np.sum(np.power(W2,2))  +  np.sum(np.power(W3,2)) 
        # loss = np.array([loss, reg * regularization_loss])
        ...
        # dLi/dprobs 
        dprobs = probs
        dprobs[np.arange(probs.shape[0]), y] -= 1
        dprobs /= X.shape[0] 
        
        # dLi/dW3 and dLi/db3
        grads["W3"]  = np.dot(hidden_layer_2.T, dprobs) 
        grads["b3"]  = np.sum(dprobs, axis=0, keepdims=True)

        # dLi/dhidden_layer2
        dhidden_layer2 = np.dot(dprobs,W3.T)

        # dLi/drelu
        dhidden_layer2[hidden_layer_2 <= 0] = 0
        
        # dLi/dW2 and dLi/db2  
        grads["W2"] = np.dot(hidden_layer_1.T,dhidden_layer2)
        grads["b2"] = np.sum(dhidden_layer2, axis=0, keepdims=True)

        # dLi/dhidden_layer1
        dhidden_layer1 = np.dot(dhidden_layer2,W2.T) 
        dhidden_layer1[hidden_layer_1 <= 0] = 0

        grads["W1"] = np.dot(X.T, dhidden_layer1) 
        grads["b1"] = np.sum(dhidden_layer1, axis=0, keepdims=True)
    ...
    def predict(self, X):
        ...
        W1,W2,W3,b1,b2,b3 = self.params["W1"],self.params["W2"],self.params["W3"],self.params["b1"],self.params["b2"],self.params["b3"] 

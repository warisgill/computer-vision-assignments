...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # Step:1 Affine Forward
        h0, cache_1 = affine_forward(features, W_proj, b_proj)
        # Step:2 Word Embedding Forward
        x, cache_of_embedding = word_embedding_forward(captions_in, W_embed)
        #Step 3: Vanilla RNN
        h, cache_of_rnn = rnn_forward(x, h0, Wx, Wh, b)
        #Step 4: Temporal Affine Transformation
        scores, cache_of_scores = temporal_affine_forward(h, W_vocab, b_vocab)
        #Step 5: Temporal Softmax Loss
        loss, dout = temporal_softmax_loss(scores, captions_out, mask)

        # ********************** Backward Pass *******************

        dout, dW_vocab, db_vocab  = temporal_affine_backward(dout, cache_of_scores) 

        dx, dh0, dWx, dWh, db = rnn_backward(dout,cache_of_rnn)

        dW_embed = word_embedding_backward(dx,cache_of_embedding)

        _, dW_proj, db_proj = affine_backward(dh0, cache_1)
